...
exports.inputHistoryPayment = (req, res, next) => {
    const {trx_id, bill_no, payment_date} = req.body;
    ...
    CustomerSub.selectTagihanFromVa(selectFromVA, (err, data) => {
        if(err){
            ...
            const inputIntoHistory = [trx_id, personalId, payment_date, payment_date];

            CustomerSub.insertHistoryPayment(inputIntoHistory, (err, result) => {
                if(err){
                    res.send('Error: ', err);
                }else{
                    next();
...
exports.inputHistoryPaymentTest = (req, res, next) => {
    const {trx_id, bill_no, payment_date} = req.body;
    selectFromVA = [bill_no];
    CustomerSub.selectTagihanFromVa(selectFromVA, (err, data) => {
        if(err){
...
exports.createPPPoEAccount = async (req, res, next) => {
    const {bill_no} = req.body;
    CustomerSub.selectPaketUser(bill_no, async(err, result) => {
        if(err){
            res.send(err);
            ...
            const idLayanan = result.id_layanan;
            const personalId = result.personal_id;
            await clientRosRest.add('/user-manager/user', {
                name: personalId,
                password: personalId,
                group: idLayanan
            });
